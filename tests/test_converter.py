...
# Rejection/rollback tests use __wrapped__ to retain coverage of the inner
# pipeline's checks before the public original-input fallback handles failures.


@pytest.mark.parametrize("level", ["2a", "2b", "2u", "3a", "3b", "3u"])
def test_successful_conversion_does_not_use_original_fallback(
    sample_pdf: Path, tmp_path: Path, level: str
) -> None:
    result = convert_to_pdfa(sample_pdf, tmp_path / "output.pdf", level=level)
    assert result.success and result.published and result.target_produced
    assert not result.skipped and not result.validation_failed
...
@pytest.mark.parametrize(
    "failure_target",
    [
        "pdftopdfa.converter._validate_ocr_configuration",
        "pdftopdfa.converter.pikepdf.open",
        "pdftopdfa.converter.check_font_compliance",
        "pdftopdfa.converter.sanitize_for_pdfa",
        "pdftopdfa.converter.embed_color_profiles",
        "pdftopdfa.converter.ensure_logical_structure",
        "pdftopdfa.converter.save_pdfa",
        "pdftopdfa.converter._validate_pdfa_output",
        "pdftopdfa.ocr.apply_ocr",
    ],
)
def test_any_conversion_stage_failure_preserves_input(
    sample_pdf: Path, tmp_path: Path, failure_target: str
) -> None:
    output = tmp_path / "output.pdf"
    output.write_bytes(b"previous output")
    original = sample_pdf.read_bytes()
    options = {}
    if failure_target == "pdftopdfa.ocr.apply_ocr":
        options = {
            "ocr_detection_model_dir": _DETECTION_MODEL_DIR,
            "ocr_recognition_model_dir": _RECOGNITION_MODEL_DIR,
        }

    with patch(failure_target, side_effect=RuntimeError("stage failed")) as failing:
        result = convert_to_pdfa(
            sample_pdf, output, level="3a", validate=True, **options
        )

    failing.assert_called()
    assert result.success and result.skipped and result.published
    assert not result.target_produced
    assert result.level is None
    assert any("stage failed" in warning for warning in result.warnings)
    assert output.read_bytes() == original == sample_pdf.read_bytes()
...
def test_failed_original_copy_returns_error_without_replacing_destination(
    ...
    with (
        patch(
            "pdftopdfa.converter.sanitize_for_pdfa", side_effect=RuntimeError("stage")
        ),
        patch(
            "pdftopdfa.converter._copy_input_to_output",
            side_effect=PermissionError("destination locked"),
        ),
    ):
...
def test_cancelled_conversion_is_not_caught(sample_pdf: Path, tmp_path: Path) -> None:
    with (
        patch("pdftopdfa.converter.sanitize_for_pdfa", side_effect=KeyboardInterrupt),
        pytest.raises(KeyboardInterrupt),
    ):
        convert_to_pdfa(sample_pdf, tmp_path / "output.pdf")
...
def test_initial_publication_failure_cleans_backup(
    ...
    with pytest.raises(PermissionError, match="destination locked"):
        convert_to_pdfa.__wrapped__(sample_pdf, output, pdfa=not processing_only)
...
def test_unreconstructable_annotation_preserves_existing_destination(
    ...
    with pytest.raises(ConversionError, match="without a normal appearance"):
        convert_to_pdfa.__wrapped__(source, output)
...
class TestConversionResult:
    ...
    def test_audit_record_preserves_structured_evidence(self, tmp_dir: Path) -> None:
        """The result can be persisted without parsing human-readable warnings."""
        result = ConversionResult(
            success=True,
            input_path=tmp_dir / "input.pdf",
            output_path=tmp_dir / "output.pdf",
            ...
            sanitization_stats={"actions_removed": 1},
            tagging_stats={"structure_rebuilt": True},
        )

        audit = result.to_dict()

        assert audit["schema_version"] == 1
        assert audit["pdfua_status"] == "review_required"
        assert audit["review_findings"][0]["code"] == ("generated_semantic_structure")
        assert audit["validation_results"][0]["validator_version"] == "1.30.2"
        assert "raw_xml" not in audit["validation_results"][0]
        assert (
            result.to_dict(include_raw_validation=True)["validation_results"][0][
                "raw_xml"
            ]
            == "<report />"
        )
...
class TestConvertToPdfa:
    ...
    def test_rejects_document_without_pages(self, level: str, tmp_dir: Path) -> None:
        ...
        with pytest.raises(UnsupportedPDFError, match="contains no pages"):
            convert_to_pdfa.__wrapped__(input_path, output_path, level=level)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "signed",
                {"validate": True, "publication_policy": "always"},
            ),
    ...
    def test_no_processing_partial_copy_preserves_existing_output(
        ...
        with patch(
            ...
            with pytest.raises(PermissionError, match="copy interrupted"):
                convert_to_pdfa.__wrapped__(sample_pdf, output_path, pdfa=False)
    ...
    def test_partial_pdf_save_preserves_existing_output(
        ...
        with patch.object(pikepdf.Pdf, "save", partial_save):
            with pytest.raises(PermissionError, match="disk full"):
                convert_to_pdfa.__wrapped__(sample_pdf, output_path, level="2b")
    ...
    def test_convert_nonexistent_file(self, tmp_dir: Path) -> None:
        ...
        with pytest.raises(ConversionError):
            convert_to_pdfa.__wrapped__(nonexistent, output_path)
    ...
    def test_convert_invalid_level_raises_error(self, tmp_dir: Path) -> None:
        ...
        with pytest.raises(ConversionError, match="Invalid PDF/A level"):
            convert_to_pdfa.__wrapped__(input_path, output_path, level="invalid")
    ...
    def test_convert_pdfua_adds_identification_and_catalog_requirements(
        ...
        assert result.success is True
        assert result.pdfua_status is PDFUAStatus.REVIEW_REQUIRED
        assert result.review_required is True
        assert [item.profile for item in result.validation_results] == [level, "ua1"]
        finding_codes = {finding.code for finding in result.review_findings}
        assert "generated_semantic_structure" in finding_codes
        assert "fallback_document_title" in finding_codes
        assert "undetermined_document_language" in finding_codes
        assert "human_accessibility_review" in finding_codes
        assert result.metadata_sources == {
            "title": "fallback",
            "language": "fallback",
        }
        with Pdf.open(output_path) as pdf:
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_pdfua_authoritative_title_and_language_are_applied(
        self, mock_verapdf: MagicMock, sample_pdf: Path, tmp_dir: Path
    ) -> None:
        """Author-supplied accessibility metadata replaces technical fallbacks."""
        mock_verapdf.side_effect = lambda *, path, flavour: VeraPDFResult(
            compliant=True,
            flavour=flavour,
        )
        ...
        finding_codes = {finding.code for finding in result.review_findings}
        assert "fallback_document_title" not in finding_codes
        assert "undetermined_document_language" not in finding_codes
        assert result.metadata_sources == {"title": "user", "language": "user"}
        with Pdf.open(output_path) as pdf:
            assert str(pdf.Root.Lang) == "en-GB"
            assert str(pdf.docinfo.Title) == "Annual report"

    @pytest.mark.parametrize("language", ["", "en_uk", "not a tag"])
    def test_rejects_invalid_document_language(
        self, sample_pdf: Path, tmp_dir: Path, language: str
    ) -> None:
        """Catalog language overrides must be valid BCP 47 tags."""
        with pytest.raises(ConversionError, match="BCP 47"):
            convert_to_pdfa.__wrapped__(
                sample_pdf,
                tmp_dir / "output.pdf",
                level="2a",
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  <rdf:RDF xmlns:rdf="{NAMESPACES["rdf"]}"
    ...
    def test_convert_removes_unrequested_pdfua_claim_without_tag_rebuild(
        self, sample_pdf: Path, tmp_dir: Path
    ) -> None:
        """A stale PDF/UA claim never survives a non-PDF/UA conversion."""
        input_path = tmp_dir / "claimed_pdfua.pdf"
        output_path = tmp_dir / "output.pdf"
        xmp = f"""\
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  <rdf:RDF xmlns:rdf="{NAMESPACES["rdf"]}"
    ...
    <rdf:Description rdf:about="">
      <pdfuaid:part>1</pdfuaid:part>
        ...
        result = convert_to_pdfa(input_path, output_path, level="3b")
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_unrequested_pdfua_claim_cannot_bypass_removal_via_pdfa_skip(
        self, mock_verapdf: MagicMock, sample_pdf: Path, tmp_dir: Path
    ) -> None:
        """A claimed PDF/A input is rewritten when its UA claim was not requested."""
        input_path = tmp_dir / "claimed_pdfa_and_pdfua.pdf"
        output_path = tmp_dir / "output.pdf"
        xmp = f"""\
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  <rdf:RDF xmlns:rdf="{NAMESPACES["rdf"]}"
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_unchanged_valid_pdfua_has_machine_validated_status(
        self, mock_verapdf: MagicMock, sample_pdf: Path, tmp_dir: Path
    ) -> None:
        """An exact already-conforming copy is distinct from newly authored tags."""
        input_path = tmp_dir / "claimed_pdfa_and_pdfua.pdf"
        output_path = tmp_dir / "output.pdf"
        xmp = f"""\
<x:xmpmeta xmlns:x="adobe:ns:meta/">
    ...
    <rdf:Description rdf:about="">
      ...
      <pdfuaid:part>1</pdfuaid:part>
        ...
        with Pdf.open(sample_pdf) as pdf:
            embed_xmp_metadata(pdf, xmp)
            pdf.save(input_path)
        mock_verapdf.side_effect = lambda path, *, flavour, **kwargs: VeraPDFResult(
            compliant=True,
            flavour=flavour,
        )
        ...
        assert result.skipped is True
        assert result.pdfua_status is PDFUAStatus.MACHINE_VALIDATED
...
<x:xmpmeta xmlns:x="adobe:ns:meta/">
  <rdf:RDF xmlns:rdf="{NAMESPACES["rdf"]}"
    ...
    def test_convert_with_failing_validation_sets_flag(
        ...
        """Non-compliance leaves the existing canonical output untouched."""
        ...
        sentinel = b"existing output"
        output_path.write_bytes(sentinel)
        result = convert_to_pdfa.__wrapped__(sample_pdf, output_path, validate=True)
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_convert_with_unavailable_validation_withholds_output(
        self,
        mock_verapdf: MagicMock,
        sample_pdf: Path,
        tmp_dir: Path,
    ) -> None:
        """A validator infrastructure error also fails closed."""
        ...
        assert result.validation_results == (
            ProfileValidationResult(profile="3b", error="veraPDF crashed"),
        )
        assert any("not published" in warning for warning in result.warnings)
        validated_path = mock_verapdf.call_args.kwargs["path"]
        assert validated_path != output_path
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_convert_can_explicitly_publish_noncompliant_candidate(
        self, mock_verapdf: MagicMock, sample_pdf: Path, tmp_dir: Path
    ) -> None:
        """The unsafe publication behavior requires an explicit policy."""
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_signed_validation_failure_preserves_destination(
        self,
        mock_verapdf: MagicMock,
        sample_pdf: Path,
        tmp_dir: Path,
    ) -> None:
        """A signed input cannot bypass fail-closed PDF/A validation."""
        signed_input = tmp_dir / "signed_input.pdf"
        _write_signed_pdf(sample_pdf, signed_input)
        output_path = tmp_dir / "output.pdf"
        sentinel = b"existing output"
        ...
        assert result.validation_results == (
            ProfileValidationResult(
                profile="3b",
                error=(
                    "PDF/A-3b validation could not run because conversion would "
    ...
    @patch("pdftopdfa.converter.validate_with_verapdf")
    def test_signed_validation_failure_can_be_explicitly_published(
        self,
        mock_verapdf: MagicMock,
        sample_pdf: Path,
        tmp_dir: Path,
    ) -> None:
        """Best-effort publication retains failure state for signed input."""
        signed_input = tmp_dir / "signed_input.pdf"
        _write_signed_pdf(sample_pdf, signed_input)
        output_path = tmp_dir / "output.pdf"
    ...
    def test_corrupt_pdf_raises_conversion_error(self, tmp_dir: Path) -> None:
        ...
        with pytest.raises(ConversionError, match="PDF processing error"):
            convert_to_pdfa.__wrapped__(corrupt_path, output_path)
    ...
    def test_output_permission_error_is_not_wrapped(
        ...
        with pytest.raises(PermissionError, match="access denied"):
            convert_to_pdfa.__wrapped__(sample_pdf, tmp_dir / "output.pdf")
...
class TestConvertDirectory:
    ...
    def test_convert_directory_with_ocr_languages(
        ...
        assert any("OCR not available" in warning for warning in results[0].warnings)
        assert results[0].output_path.read_bytes() == sample_pdf_bytes
...
class TestConvertFiles:
    ...
    def test_convert_files_error_continues(
        ...
        assert len(results) == 3
        assert results[0].success is True
        assert results[1].success and results[1].skipped
        assert not results[1].target_produced
        assert results[1].warnings
        assert out_bad.read_bytes() == bad.read_bytes()
        assert results[2].success is True

    @patch("pdftopdfa.converter.convert_to_pdfa")
    def test_convert_files_permission_error_continues(
        self, mock_convert_to_pdfa: MagicMock, tmp_dir: Path
    ) -> None:
        """A permission failure on one file does not abort the batch."""
        first_input = tmp_dir / "first.pdf"
        first_output = tmp_dir / "first_pdfa.pdf"
        second_input = tmp_dir / "second.pdf"
        second_output = tmp_dir / "second_pdfa.pdf"
        mock_convert_to_pdfa.side_effect = [
